...
<script type="text/javascript">
	$(function() {
		...
		$("#addColumn").click(
				function() {
					overrideSelectedTabForSubPage('addColumn', '新增栏目',
							'admin/column.do?action=toAddColumn');
				});
		...
		$("#updateColumn")
				.click(
						function() {
							...
							overrideSelectedTabForSubPage('updateColumn',
									'修改栏目',
									'admin/column.do?action=toUpdateColumn&id='
											+ checkListValue);
		...
		$("#toSearch").click(function() {
			...
			var formAction = "column.do?action=searchColumn";
		...
		//导出栏目
		$("#toExportColumnList").click(function() {
			var formToExport = document.getElementById("queryForm");

			var formAction = "column.do?action=exportColumnList";
		...
		function getFormActionCriteria() {
			var formAction = "";
			formAction += formAction += "&parentId=" + $("#parentId").val();
			formAction += "&name=" + $("#name").val();
			formAction += "&code=" + $("#code").val();
		...
		$(".update").click(
				function() {
					overrideSelectedTabForSubPage('updateColumn', '修改栏目',
							'admin/column.do?action=toUpdateColumn&id='
									+ $(this).attr("value"));
				});
		...
		$(".view").click(
				function() {
					overrideSelectedTabForSubPage('viewColumn', '查看栏目',
							'admin/column.do?action=viewColumn&id='
									+ $(this).attr("value"));
				});
...
<body>
	<div class="functionList">
		...
		<div class="button1" id="addColumn">
	...
	<%-- <form action="${ctx }/admin/column.do?action=searchColumn"
		...
		<table width="100%" border="1" class="formTable" id="searchTable">
			...
			<tr>
				<td class="fieldName" width="10%">code:</td>
				<td class="fieldForm" width="40%"><input type="text"
					name="code" id="code" value="${bean.code  }" /></td>
			...
			<tr>
				<td colspan="4" align="center"><input type="submit" value="搜索"
					...
					type="button" value="导出" id="toExportColumnList"
					name="toExportColumnList" class="submitButton" /> <input
	...
	<table width="100%" id="mytab" border="1" class="t1">
		...
		<c:forEach items="${columnList }" var="columnList" varStatus="vs">
			<tr <c:if test="${vs.index %2==1 }">
					class="a1"
				</c:if>>
				<td><input type="checkbox" class="checkList" name="checkList"
					value="${columnList.id }" /></td>
					<td><c:if test="${columnList.level ==1 }">
								≡&nbsp;${columnList.name }
								</c:if> <c:if test="${columnList.level ==2 }">
								&nbsp;&nbsp;&nbsp;└&nbsp;${columnList.name }
								</c:if> <c:if test="${columnList.level ==3 }">
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└&nbsp;${columnList.name }
								</c:if> <c:if test="${columnList.level ==4 }">
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└&nbsp;${columnList.name }
								</c:if> <c:if test="${columnList.level ==5 }">
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└&nbsp;${columnList.name }
								</c:if></td>
				<td><span class="update operationButton" value="${columnList.id }">修改</span>
					<span class="operationButton">|</span> <span
					class="delete operationButton" value="${columnList.id }">删除</span><span
					class="operationButton">|</span> <span class="view operationButton"
					value="${columnList.id }">查看</span></td>
			</tr>
		</c:forEach>
	</table>
</body>
