...
<script type="text/javascript">
	$(function() {
		...
		$("#addUserGroup")
				.click(
						function() {
							overrideSelectedTabForSubPage('addUserGroup',
									'新增用户组',
									'admin/userGroup.do?action=toAddUserGroup');
						});
				...
				.click(
						function() {
							...
							$
									.ajax({
										type : "POST",
										url : "userGroup.do?action=deleteUserGroup&randomNum="
												+ new Date().getTime()
												+ Math.random(),
										cache : false,
				...
				.click(
						function() {
							...
							overrideSelectedTabForSubPage(
									'updateUserGroup',
									'修改用户组',
									'admin/userGroup.do?action=toUpdateUserGroup&id='
											+ checkListValue);
		...
		$("#toSearch")
				.click(
						function() {
							...
							var formAction = "userGroup.do?action=searchUserGroup";
		...
		//导出用户组
		$("#toExportUserGroupList")
				.click(
						function() {
							var formToExport = document
									.getElementById("queryForm");

							var formAction = "userGroup.do?action=exportUserGroupList";
  ...
  formAction  +=  "&groupId="+$("#groupId").val();
		...
		$(".update").click(
				function() {
					overrideSelectedTabForSubPage('updateUserGroup',
							'修改用户组',
							'admin/userGroup.do?action=toUpdateUserGroup&id='
									+ $(this).attr("value"));
				});
		...
		$(".view").click(
				function() {
					overrideSelectedTabForSubPage('viewUserGroup',
							'查看用户组',
							'admin/userGroup.do?action=viewUserGroup&id='
									+ $(this).attr("value"));
				});
...
<body>
	<div class="functionList">
		...
		<div class="button1" id="addUserGroup">
			<div class="button1Left"></div>
			新增
			<div class="button1Right"></div>
		</div>

		<div class="button1" id="updateUserGroup">
			<div class="button1Left"></div>
			修改
			<div class="button1Right"></div>
		</div>

		<div class="button1" id="deleteUserGroup">
			<div class="button1Left"></div>
			删除
			<div class="button1Right"></div>
		</div>
	...
	<form
		action="${ctx }/admin/userGroup.do?action=searchUserGroup"
		name="queryForm" id="queryForm" method="post">
		<table width="100%" border="1" class="formTable" id="searchTable">
			<tr><td  class="fieldName"  width="10%">userId:</td> <td  class="fieldForm"  width="40%"><input  type="text"         name="userId"  id="userId"  value="${bean.userId  }"  /></td><td  class="fieldName"  width="10%">groupId:</td> <td  class="fieldForm"  width="40%"><select name="groupId" id="groupId"><option value="all" selected="selected">全部</option><c:forEach var="groupList" items="${groupList }"><option <c:if test="${bean.groupId ==groupList.id  }"> selected="selected"</c:if>value="${groupList.id }">${groupList.name }</option></c:forEach></select> </td></tr>
			<tr>
				<td colspan="4" align="center"><input type="submit" value="搜索"
					id="toSearch" name="toSearch" class="submitButton" /> <input type="button"
					value="导出" id="toExportUserGroupList" name="toExportUserGroupList" class="submitButton" /> <input
	...
	<table width="100%" id="mytab" border="1" class="t1">
		<thead>
			<tr>
				<th><input type="checkbox" id="checkAll" /></th> <th>user_id</th><th>group_id</th>
				<th>操作</th>
			</tr>
		</thead>
		...
		<c:forEach items="${page.pageDatas }" var="list" varStatus="vs">
			<tr <c:if test="${vs.index %2==1 }">
				...
				<td><input type="checkbox" class="checkList" name="checkList"
					value="${list.id }" /></td> <td>${list.user_id }</td><td>${list.group_id }</td>
	...
	<div class="pageBar">
		<tkxgz:page
			url="${ctx }/admin/userGroup.do?action=listUserGroup"
			page="${page }" />
	</div>
</body>
</html>
