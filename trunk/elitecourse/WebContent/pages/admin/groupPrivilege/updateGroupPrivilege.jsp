...
<body>
	<form action="${ctx }/admin/groupPrivilege.do?action=updateGroupPrivilege"
		...
		<table width="100%" border="1" class="formTable">
			...
			<tr><td  class="fieldName"  width="20%">groupId:</td>  <td  class="fieldForm"  width="80%"><select name="groupId" id="groupId"><option value="">请选择</option><c:forEach var="groupList" items="${groupList }"><option value="${groupList.id }" <c:if test="${map.group_id ==groupList.id  }"> selected="selected"</c:if> >${groupList.name }</option></c:forEach></select><span  class="asterisk">*</span></td> </tr><tr><td  class="fieldName"  width="20%">privilegeId:</td>  <td  class="fieldForm"  width="80%"><select name="privilegeId" id="privilegeId"><option value="">请选择</option><c:forEach var="privilegeList" items="${privilegeList }"><option value="${privilegeList.id }" <c:if test="${map.privilege_id ==privilegeList.id  }"> selected="selected"</c:if> >${privilegeList.name }</option></c:forEach></select><span  class="asterisk">*</span></td> </tr>
