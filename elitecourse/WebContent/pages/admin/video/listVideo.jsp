...
<script type="text/javascript">
	$(function() {
		...
		$("#addVideo")
				.click(
						function() {
							overrideSelectedTabForSubPage('addVideo',
									'新增视频',
									'admin/video.do?action=toAddVideo');
						});
				...
				.click(
						function() {
							...
							$
									.ajax({
										type : "POST",
										url : "video.do?action=deleteVideo&randomNum="
												+ new Date().getTime()
												+ Math.random(),
										cache : false,
				...
				.click(
						function() {
							...
							overrideSelectedTabForSubPage(
									'updateVideo',
									'修改视频',
									'admin/video.do?action=toUpdateVideo&id='
											+ checkListValue);
		...
		$("#toSearch")
				.click(
						function() {
							...
							var formAction = "video.do?action=searchVideo";
		...
		$("#toExportVideoList")
				.click(
						function() {
							...
							var formAction = "video.do?action=exportVideoList";
	...
	/*   formAction  +=  "&source="+$("#source").val(); */
		...
		$(".update").click(
				function() {
					overrideSelectedTabForSubPage('updateVideo', '修改视频',
							'admin/video.do?action=toUpdateVideo&id='
									+ $(this).attr("value"));
				});
		...
		$(".view").click(
				function() {
					overrideSelectedTabForSubPage('viewVideo', '查看视频',
							'admin/video.do?action=viewVideo&id='
									+ $(this).attr("value"));
				});
...
<body>
	<div class="functionList">
		...
		<div class="button1" id="addVideo">
			<div class="button1Left"></div>
			新增
			<div class="button1Right"></div>
		</div>
<!-- 
		<div class="button1" id="updateVideo">
			<div class="button1Left"></div>
			修改
			<div class="button1Right"></div>
		</div> -->

		<div class="button1" id="deleteVideo">
			<div class="button1Left"></div>
			删除
			<div class="button1Right"></div>
		</div>
		...
		<table width="100%" border="1" class="formTable" id="searchTable">
			...
			<tr>
				<td colspan="4" align="center"><input type="submit" value="搜索"
					...
					type="button" value="导出" id="toExportVideoList"
					name="toExportVideoList" class="submitButton" />  --> <input
		...
		<c:forEach items="${page.pageDatas }" var="list" varStatus="vs">
			<tr <c:if test="${vs.index %2==1 }">
				...
				<td><input type="checkbox" class="checkList" name="checkList"
					value="${list.id }" /></td>
				<td>${list.name }</td>
				<td><c:if test="${list.type =='FLV' }">FLV</c:if> <c:if
						test="${list.type =='RM' }">RM</c:if> <c:if
						test="${list.type =='FLASH' }">FLASH</c:if> <c:if
						test="${list.type =='REAL' }">REAL</c:if></td>
				<td>
					<%-- <span class="update operationButton" value="${list.id }">修改</span>
					...
					value="${list.id }">查看</span>
				</td>
			</tr>
		</c:forEach>
	</table>
	<div class="pageBar">
		<tkxgz:page url="${ctx }/admin/video.do?action=listVideo"
			page="${page }" />
	</div>
</body>
</html>
